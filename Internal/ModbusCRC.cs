using System;
using System.Collections.Generic;
using System.Linq;
using System.Security.Cryptography;
using System.Text;

/****************************************************************
*  Copyright © (2023) www.eelf.cn All Rights Reserved.          *
*  Author : jacky                                               *
*  QQ : 7092734                                                 *
*  Email : jacky@eelf.cn                                        *
*  Site : www.eelf.cn                                           *
*  Create Time : 2023-12-19 10:44:18                            *
*  Version : v 1.0.0                                            *
*  CLR Version : 4.0.30319.42000                                *
*****************************************************************/
namespace XiaoFeng.Modbus.Internal
{
    /// <summary>
    /// ModbusCRC 类说明
    /// </summary>
    public static class ModbusCRC
    {
        #region 属性
        /// <summary>
        /// CRC高位校验码
        /// </summary>
        private static readonly byte[] AucCRCHigh =
        {
            
          };
        /// <summary>
        /// CRC低位校验码
        /// </summary>
        private static readonly byte[] AucCRCLow =
         {
            
          };
        /// <summary>
        /// 参数
        /// </summary>
        private static readonly UInt16[] crc_ta = new UInt16[16] { 0x0000, 0xCC01, 0xD801, 0x1400, 0xF001, 0x3C00, 0x2800, 0xE401, 0xA001, 0x6C00, 0x7800, 0xB401, 0x5000, 0x9C01, 0x8801, 0x4400, };

        #endregion

        #region 方法
        /// <summary>Crc校验</summary>
        /// <param name="data"></param>
        /// <param name="offset">偏移</param>
        /// <param name="length">数量</param>
        /// <returns></returns>
        public static ushort CRC(byte[] data, int offset, int length = -1)
        {
            if (data == null || data.Length < 1) return 0;

            ushort u = 0xFFFF;
            byte b;

            if (length == 0) length = data.Length - offset;

            for (var i = offset; i < length; i++)
            {
                b = data[i];
                u = (ushort)(crc_ta[(b ^ u) & 15] ^ (u >> 4));
                u = (ushort)(crc_ta[((b >> 4) ^ u) & 15] ^ (u >> 4));
            }
            return u;
        }
        /// <summary>
        /// 生成CRC
        /// </summary>
        /// <param name="data">字节组</param>
        /// <param name="offset">开始位置</param>
        /// <param name="length">长度</param>
        /// <returns></returns>
        public static byte[] CRC16(byte[] data, int offset, int length = -1)
        {
            if (data == null || data.Length == 0 || length == 0 || (length > -1 && data.Length < offset + length)) return data;
            if (length == -1) length = data.Length - offset;
            int i = 0;
            var bytes = new byte[length];
            Array.Copy(data, offset, bytes, 0, length);
            var _data = new byte[2] { 0xFF, 0xFF };
            var index = 0;
            while (length-- > 0)
            {
                index = (ushort)(_data[0] ^ bytes[i++]);
                _data[0] = (byte)(_data[1] ^ AucCRCHigh[index]);
                _data[1] = AucCRCLow[index];
            }
            return data;
        }
        /// <summary>
        /// 纵向冗余校验LRC
        /// </summary>
        /// <param name="data">原数组</param>
        /// <param name="offset">从头开始偏移几个byte</param>
        /// <param name="length">偏移后取几个字节byte</param>
        /// <returns>8位LRC</returns>
        public static byte LRCCalc(byte[] data, int offset, int length)
        {
            byte lrc = 0;
            for (int i = 0; i < length; i++)
                lrc += data[i + offset];
            return (byte)(-(sbyte)lrc);
        }

        /// <summary>
        /// 纵向冗余校验LRC
        /// </summary>
        /// <param name="data">原数组（有效字符串，不能含帧头和帧尾）</param>
        /// <param name="offset">从头开始偏移几个byte</param>
        /// <param name="length">偏移后取几个字节byte</param>
        /// <returns>8位LRC</returns>
        public static byte LRCCalc(string data, int offset, int length)
        {
            try
            {
                data = new string(data.Replace(" ", string.Empty).ToCharArray().Skip(offset).Take(length).ToArray());

                if (data.Length % 2 == 1)
                    throw new ArgumentException();

                byte[] buffer = new byte[data.Length / 2];
                for (int i = 0; i < data.Length; i += 2)
                    buffer[i / 2] = Convert.ToByte(data.Substring(i, 2), 16);

                return LRCCalc(buffer, 0, buffer.Length);
            }
            catch
            {
                return 0;
            }
        }
        #endregion
    }
}